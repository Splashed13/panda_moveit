...
class PickAndPlace(object):
    ...
    def create_collision_scene(self) -> None:
        ...
        self.scene.add_object(self.create_collision_object("floor", [2.5, 2.5, 0.01], [0, 0, -0.01], 0))
        self.scene.add_object(self.create_collision_object("box1", [0.2, 0.4, box1["z_height"]], [box1["x_pos"], box1["y_pos"], box1["z_height"]/2], 0))
        self.scene.add_object(self.create_collision_object("box2", [0.3, 0.2, box2["z_height"]], [box2["x_pos"], box2["y_pos"], box2["z_height"]/2], 0))
        self.scene.add_object(self.create_collision_object("rod", [0.02, 0.02, rod_height], [box1["x_pos"], box1["y_pos"], rod_height/2 + box1["z_height"]], 45))
    ...
    def close_gripper(self) -> None:
        """Close the gripper"""
        print(f"[INFO] Current End Effector Joint Values (Closed): {[round(item, 3) for item in self.end_effector.get_current_joint_values()]}")
    ...
    def open_gripper(self) -> None:
        """Open the gripper"""
        print(f"[INFO] Current End Effector Joint Values (Open): {[round(item, 3) for item in self.end_effector.get_current_joint_values()]}")
    ...
    def place_rod(self) -> None:
        place_point_position = [box2["x_pos"], box2["y_pos"], rod_height/2 + box2["z_height"]]
...
def main():
    try:
        ...
        print(f"[INFO] Home Joint Values: {[round(item, 3)for item in home_joint_pos]}\n[INFO] Setting up scene with Collision Objects")
